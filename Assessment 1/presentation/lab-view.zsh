...
build_demos() {
    ...
    case "$ACTIVITY" in
      ...
      5)
        ...
        slide "Demo 1 — EICAR test file (Part B)" "download the standard AV test file, wrap the signature in a runnable ELF, and execute it so Rustinel's scan-on-process-start fires the EICAR YARA rule" text "echo '== download the standard EICAR test file =='; wget -qO ~/eicar.com https://secure.eicar.org/eicar.com.txt && echo 'downloaded ~/eicar.com' || { echo 'wget failed - writing the signature offline'; echo WDVPIVAlQEFQWzRcUFpYNTQoUF4pN0NDKTd9JEVJQ0FSLVNUQU5EQVJELUFOVElWSVJVUy1URVNULUZJTEUhJEgrSCo= | base64 -d > ~/eicar.com; }; echo; echo '== the raw EICAR file is not a Linux executable, and Rustinel scans on process-start, so wrap the signature in a runnable ELF and run THAT =='; cp /bin/true ~/eicar_run; cat ~/eicar.com >> ~/eicar_run; chmod +x ~/eicar_run; ~/eicar_run && echo 'ran ~/eicar_run (its process image carries the EICAR signature)'; sleep 2; echo; echo '== Rustinel alerts =='; sudo /usr/local/sbin/labview-rustinel-alerts 15"
        ...
        slide "Demo 3 — SSH brute force (Part D)" "enable sshd password auth for the test -> 8 wrong-password logins to localhost -> custom unix_chkpwd Sigma rule (burst) -> disable password auth again" text "echo '== enable sshd password auth (for the test only) =='; sudo /usr/local/sbin/labview-ssh-passwordauth on; sleep 1; echo; echo '== send 8 wrong-password SSH logins to localhost =='; if command -v sshpass >/dev/null 2>&1; then for i in 1 2 3 4 5 6 7 8; do sshpass -p wrong\$i ssh -o StrictHostKeyChecking=no -o PreferredAuthentications=password -o PubkeyAuthentication=no -o ConnectTimeout=3 \$USER@localhost true 2>/dev/null; done; echo '8 failed attempts sent'; else echo 'sshpass missing - sudo apt-get install -y sshpass'; fi; sleep 2; echo; echo '== Rustinel alerts (brute-force burst) =='; sudo /usr/local/sbin/labview-rustinel-alerts 30; echo; echo '== disable sshd password auth again (security) =='; sudo /usr/local/sbin/labview-ssh-passwordauth off"
        ;;
...
show_host() {
    ...
    if [[ "$ACTIVITY" == 5 ]]; then
        ...
        hl "$(run "$H" "rm -f ~/eicar.com ~/eicar_run && echo 'removed ~/eicar.com and ~/eicar_run'; sudo /usr/local/sbin/labview-ssh-passwordauth off; sudo /usr/local/sbin/labview-rustinel-reset")" text
        wait_for_enter
    fi
}
