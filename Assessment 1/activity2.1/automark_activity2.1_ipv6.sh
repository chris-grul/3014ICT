...
#   E20 — IPv6 tunnel service failing on the Remote Gateway (wstunnel/wg-quick
#         not active, or no WireGuard handshake on wg0)
#
# This script can also run on the Remote Gateway (BinaryLane VM) — the IPv6
# tunnel endpoint, public IPv4 112.213.39.252.
#
# Usage: sudo ./automark_activity2.1_ipv6.sh
# =============================================================================
...
run_remote_gateway() {
    ...
    for _svc in wstunnel-server wg-quick@wg0; do
        ...
        else fail "E20" "service $_svc is not active"; fi
    done
    ...
    else fail "E20" "no WireGuard handshake on wg0 — tunnel not established"; fi
