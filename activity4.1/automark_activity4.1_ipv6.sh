...
#   E19 — IPv6 tunnel services not running (Remote Gateway: wstunnel-server /
#         wg-quick@wg0 inactive, or no WireGuard handshake on wg0)
#
# This script can run on: External Gateway, and the Remote Gateway
# (BinaryLane VM — public IPv4 112.213.39.252, the IPv6 tunnel endpoint).
#
# Usage: sudo bash automark_activity4.1_ipv6.sh
# =============================================================================
...
# =============================================================================
# Remote Gateway (BinaryLane VM — IPv6 uplink / tunnel endpoint)
# =============================================================================
run_remote_gateway() {
    ...
    for _i in eth0 wg0; do
        ...
        else fail "E13" "Interface $_i not found"; fi
    done

    section "IPv4 Addressing"
    if has_ip "112.213.39.252"; then pass "eth0 has public IPv4 (112.213.39.252)"
    else fail "E13" "public IPv4 112.213.39.252 not found"; fi

    section "IPv6 Addressing"
    check_ip6 "wg0" "2404:9400:29c1:df00::254/64" "E13"
    check_ip6_has_global "eth0" "E13"

    section "IPv6 Routing"
    check_route6_get "2404:9400:29c1:df10::80" "dev wg0"  "E14" "lab /56 (down the tunnel)"
    check_route6_get "2001:4860:4860::8888"    "dev eth0" "E14" "internet (own uplink)"
    ...
    for _svc in wstunnel-server wg-quick@wg0; do
        ...
        else fail "E19" "service $_svc is not active"; fi
    done
    ...
    else fail "E19" "no WireGuard handshake on wg0 — tunnel not established"; fi

    section "IPv6 Connectivity — Lab"
    check_ping6 "2404:9400:29c1:df00::1"  "External Gateway (wg0)" "E17"
